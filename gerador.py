...
VERSÃO COMPLETA - COM PUBLICAÇÃO EM LOTES E CORREÇÕES
"""
...
CONFIG = {
    ...
    'email_contato': 'contato@homedecor.reviewnexus.blog',
    'publicar_por_dia': 3,
    'redes_sociais': {
...
"""
    ...
    def get_header(self, ativo="inicio", categoria_atual=None):
        ...
        for cat in categorias[:6]:
            ...
            cat_links += f'<a href="/{CONFIG["slug"]}/categoria/{cat}/" class="{ativo_cat}">{cat.title()}</a>'
        
        return f"""
<header>
    <div class="container">
        <a href="/{CONFIG['slug']}/index.html" class="logo">
            <span class="icone">{CONFIG['icone']}</span>
            <span class="nome">{CONFIG['nome_site']}</span>
        </a>
        <button class="menu-toggle" aria-label="Menu">☰</button>
        <nav>
            <a href="/{CONFIG['slug']}/index.html" class="{'ativo' if ativo=='inicio' else ''}">{t['menu_inicio']}</a>
            {cat_links}
            <a href="/{CONFIG['slug']}/sobre.html" class="{'ativo' if ativo=='sobre' else ''}">{t['menu_sobre']}</a>
            <a href="/{CONFIG['slug']}/contato.html" class="{'ativo' if ativo=='contato' else ''}">{t['menu_contato']}</a>
    ...
    <div class="container">
        <div class="footer-grid">
            ...
            <div>
                <h4>Links</h4>
                <ul>
                    <li><a href="/{CONFIG['slug']}/index.html">{t['menu_inicio']}</a></li>
                    <li><a href="/{CONFIG['slug']}/sobre.html">{t['menu_sobre']}</a></li>
                    <li><a href="/{CONFIG['slug']}/contato.html">{t['menu_contato']}</a></li>
                </ul>
            </div>
            <div>
                <h4>Legal</h4>
                <ul>
                    <li><a href="/{CONFIG['slug']}/politica-privacidade.html">{t['privacidade_titulo']}</a></li>
                    <li><a href="/{CONFIG['slug']}/cookies.html">{t['cookies_titulo']}</a></li>
                </ul>
            </div>
        </div>
...
<div class="cookies-banner">
    <p>🍪 Usamos cookies para melhorar sua experiência. <a href="/{CONFIG['slug']}/cookies.html" style="color:var(--primaria);">Saiba mais</a></p>
...
<html lang="{t['lang']}">
<head>
    {self.get_meta_tags(f"{CONFIG['nome_site']} - {titulo}", f"{titulo} - {CONFIG['nome']}", f"{CONFIG['url_base']}/{CONFIG['slug']}/{nome}.html")}
...
<h3>5. Contato</h3>
<p>Para questões sobre esta política, entre em contato através da nossa <a href="/{CONFIG['slug']}/contato.html">página de contato</a>.</p>
...
<h3>Contato</h3>
<p>Para mais informações, entre em contato através da nossa <a href="/{CONFIG['slug']}/contato.html">página de contato</a>.</p>
...
<body>
    ...
    <main class="container">
        <div style="text-align:center;padding:60px 0;">
            ...
            <a href="/{CONFIG['slug']}/index.html" class="btn">{t['voltar_inicio']}</a>
        </div>
    </main>
    {self.get_footer()}
    <script src="assets/js/script.js"></script>
    ...
    def criar_pagina_categoria(self, categoria):
        caminho = self.docs / "categoria" / categoria / "index.html"
        t = self.t
        
        artigos = self.ler_csv()
        artigos_cat = []
        for a in artigos:
            if a.get('categoria', '').lower() == categoria.lower() and a.get('status') == 'publicado':
                artigos_cat.append({
        ...
        for a in artigos_cat:
            ...
            <div class="card">
                ...
                <h3><a href="/{CONFIG['slug']}/{a['slug']}/index.html">{a['nome']}</a></h3>
                <a href="/{CONFIG['slug']}/{a['slug']}/index.html" class="btn btn-pequeno">{t['menu_inicio']} →</a>
            </div>"""
        lista += '</div>'
...
<html lang="{t['lang']}">
<head>
    {self.get_meta_tags(f"{CONFIG['nome_site']} - {categoria.title()}", f"Artigos sobre {categoria}", f"{CONFIG['url_base']}/{CONFIG['slug']}/categoria/{categoria}/")}
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="../../assets/css/style.css">
</head>
<body>
    ...
    def criar_artigo(self, artigo_data, forcar=False, revisar=True):
        ...
        url = f"{CONFIG['url_base']}/{CONFIG['slug']}/{slug}/"
        ...
        artigos_relacionados = self.get_artigos_relacionados(slug)
...
<body>
    ...
    <script src="../assets/js/script.js"></script>
        ...
        print(f"   ✅ Salvo: docs/{CONFIG['slug']}/{slug}/index.html")
        return caminho
    
    def get_artigos_relacionados(self, slug_atual):
        artigos = []
        for pasta in self.docs.iterdir():
            if pasta.is_dir() and pasta.name != slug_atual and (pasta / "index.html").exists():
                artigos.append({
                    'slug': pasta.name,
                    'nome': pasta.name.replace('-', ' ').title()
                })
        ...
        html = ""
        for a in artigos:
            html += f'<li><a href="/{CONFIG["slug"]}/{a["slug"]}/">{a["nome"]}</a></li>\n'
    ...
    def criar_index(self):
        ...
        artigos = []
        for pasta in self.docs.iterdir():
            if pasta.is_dir() and (pasta / "index.html").exists():
                artigos.append({
                    'slug': pasta.name,
                    'nome': pasta.name.replace('-', ' ').title()
                })
        ...
        artigos_ordenados = []
        csv_artigos = self.ler_csv()
        for a in artigos:
            data = "2025-01-01"
            for ca in csv_artigos:
                if self.criar_slug(ca.get('artigo', '')) == a['slug']:
                    data = ca.get('data_publicacao', '2025-01-01')
                    break
            artigos_ordenados.append({**a, 'data': data})
        
        artigos_ordenados.sort(key=lambda x: x['data'], reverse=True)
        
        if artigos_ordenados:
            lista = '<div class="grid">'
            for a in artigos_ordenados[:12]:
                img = self.gerar_imagem(a['nome'])
                data_formatada = datetime.strptime(a['data'], "%Y-%m-%d").strftime("%d/%m/%Y") if a['data'] else datetime.now().strftime("%d/%m/%Y")
                lista += f"""
                <div class="card">
                    <img src="{img}" alt="{a['nome']}" class="card-img" loading="lazy">
                    <div class="card-meta">📅 {data_formatada}</div>
                    <h3><a href="{a['slug']}/index.html">{a['nome']}</a></h3>
                    <a href="{a['slug']}/index.html" class="btn btn-pequeno">{t['menu_inicio']} →</a>
                </div>"""
            lista += '</div>'
        else:
...
<html lang="{t['lang']}">
<head>
    {self.get_meta_tags(f"{CONFIG['nome_site']} - {CONFIG['nome']}", CONFIG['descricao'], f"{CONFIG['url_base']}/{CONFIG['slug']}/")}
    <link href="https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="assets/css/style.css">
</head>
<body>
    ...
    def publicar_lotes(self):
        """Publica artigos em lotes controlados - NÃO RECRIA ARTIGOS EXISTENTES"""
        print("\n📦 PUBLICAR EM LOTES")
        print("=" * 50)
        ...
        for a in artigos:
            if a.get('status', 'rascunho').lower() != 'publicado':
                slug = self.criar_slug(a.get('artigo', ''))
                # Só adiciona se o arquivo NÃO existir
                if not (self.docs / slug / "index.html").exists():
                    pendentes.append(a)
    ...
    def publicar_natural(self, quantidade=None):
        ...
        # ATUALIZA TUDO (INCLUINDO SITEMAP)
        self.criar_index()
        self.criar_todas_categorias()
        self.criar_sitemap()
    ...
    def criar_sitemap(self):
        ...
        for pagina, prioridade in paginas:
            ...
            loc.text = f"{CONFIG['url_base']}/{CONFIG['slug']}/{pagina}"
        ...
        for cat in self.get_categorias():
            url_elem = ET.SubElement(urlset, 'url')
            loc = ET.SubElement(url_elem, 'loc')
            loc.text = f"{CONFIG['url_base']}/{CONFIG['slug']}/categoria/{cat}/"
            lastmod = ET.SubElement(url_elem, 'lastmod')
            lastmod.text = datetime.now().strftime('%Y-%m-%d')
            changefreq = ET.SubElement(url_elem, 'changefreq')
        ...
        for pasta in self.docs.iterdir():
            if pasta.is_dir() and (pasta / "index.html").exists():
                url_elem = ET.SubElement(urlset, 'url')
                loc = ET.SubElement(url_elem, 'loc')
                loc.text = f"{CONFIG['url_base']}/{CONFIG['slug']}/{pasta.name}/"
                lastmod = ET.SubElement(url_elem, 'lastmod')
                lastmod.text = datetime.now().strftime('%Y-%m-%d')
                changefreq = ET.SubElement(url_elem, 'changefreq')
                changefreq.text = 'weekly'
...
Sitemap: {CONFIG['url_base']}/{CONFIG['slug']}/sitemap.xml
""")
    ...
    def publicar_um(self):
        ...
        print(f"   Link: {CONFIG['url_base']}/{CONFIG['slug']}/{self.criar_slug(a.get('artigo'))}/")
    ...
    def despublicar_artigo(self):
        ...
        for pasta in self.docs.iterdir():
            if pasta.is_dir() and (pasta / "index.html").exists():
                publicados.append({'slug': pasta.name, 'nome': pasta.name.replace('-', ' ').title()})
    ...
    def revisar_artigo(self):
        ...
        while True:
            ...
            if opcao == "1":
                caminho = self.docs / slug / "index.html"
    ...
    def ver_paginas(self):
        ...
        print("\n📂 CATEGORIAS:")
        for cat in self.get_categorias():
            existe = "✅" if (self.docs / "categoria" / cat / "index.html").exists() else "❌"
            print(f"   {existe} /categoria/{cat}/")
    ...
    def limpar_cache(self):
        ...
        artigos = self.ler_csv()
        publicados = [a for a in artigos if a.get('status', 'rascunho').lower() == 'publicado']
        ...
        for pasta in self.docs.iterdir():
            if pasta.is_dir() and (pasta / "index.html").exists():
                shutil.rmtree(pasta)
                print(f"   🗑️ Removido: {pasta.name}")
        
        cat_path = self.docs / "categoria"
        if cat_path.exists():
            shutil.rmtree(cat_path)
            print("   🗑️ Removido: categoria/")
        
        print("\n📝 Regenerando artigos...")
        
        for i, a in enumerate(publicados, 1):
            print(f"\n[{i}/{len(publicados)}] Regenerando: {a.get('artigo')}")
            self.criar_artigo(a, forcar=True, revisar=True)
